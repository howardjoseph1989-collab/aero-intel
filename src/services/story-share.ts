...
export const shareTexts = {
  twitter: (data: StoryData) =>
    ...
    (data.cii ? `Instability: ${data.cii.score}/100 (${data.cii.level})\n` : 'Instability: Unavailable\n') +
    `${data.threats.critical > 0 ? `⚠️ ${data.threats.critical} critical threats\n` : ''}` +
    `\nGenerated by AERO INTEL`,

  whatsapp: (data: StoryData) =>
    `${toFlagEmoji(data.countryCode, '')} *${data.countryName} Intelligence Brief*\n\n` +
    (data.cii
      ? `*Instability:* ${data.cii.score}/100 (${data.cii.level})\n*Trend:* ${data.cii.trend}\n`
      : '*Instability:* Unavailable\n') +
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via AERO INTEL - Personal live intelligence dashboard`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - AERO INTEL`)}`,
