...
export function buildEmbedIframeSnippet(url: string, options: { width?: string; height?: string } = {}): string {
  ...
  const height = sanitizePixelDimension(options.height ?? '420', 120, 1200);
  return `<iframe src="${escapeAttribute(url)}" title="AERO INTEL live map" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="width:${width};height:${height}px;border:0;display:block" allowfullscreen></iframe>`;
}
