...
function warnOnDeprecatedCredential(kind: string | undefined): void {
  ...
  const what = kind === 'enterprise_key'
    ...
    : 'an AERO INTEL user API key (wm_…), which unlocks your whole paid REST allowance';
  console.warn(
    ...
    + 'AERO INTEL dashboard → Settings → Embeds.',
  );
}
...
async function bootEmbed(): Promise<void> {
  ...
  try {
    ...
    if (!params.panel) {
      mountError(root, `Unknown AERO INTEL embed panel "${params.requestedPanel}".`);
    ...
    if (getEmbedPanelFreeTier(params.panel) === null) {
      ...
      if (!apiKey) {
        mountError(root, 'This AERO INTEL panel requires an embedding API key from the partner account.');
        document.body.dataset.embedReady = 'error';
        return;
      }
      ...
      if (!entitlement.ok) {
        const denied = entitlement.status === 403
          ? 'The embedding account is not entitled to this AERO INTEL panel.'
          : 'AERO INTEL could not verify the embedding API key for this panel.';
    ...
    document.title = params.panel === 'map'
      ? 'AERO INTEL Live Map Embed'
      : 'AERO INTEL Panel Embed';
    ...
    mountError(root, 'AERO INTEL embed could not load.');
    document.body.dataset.embedReady = 'error';
  }
}
